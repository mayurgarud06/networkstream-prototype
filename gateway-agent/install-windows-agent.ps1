...
if (-not $python) {
    throw "Python was not found. Install Python 3 and make python.exe or py.exe available on PATH."
}
...
Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Description "NetworkStream gateway observation agent" -Force | Out-Null
