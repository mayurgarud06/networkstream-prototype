...
def scan_wifi():
 ...
 for raw in result.stdout.splitlines():
  ...
  if match and current_ssid is not None:
   ...
   if current_encryption and current_encryption.lower()!="none": security=f"{current_auth}/{current_encryption}"
   current={"ssid":current_ssid,"bssid":match.group(1).lower(),"signalDbm":None,"signalPercent":None,"frequency":None,"security":security}; networks.append(current); continue
  if current is None: continue
  match=re.match(r"Signal\s*:\s*(\d+)%",line,re.I)
  if match: current["signalDbm"]=None; current["signalPercent"]=int(match.group(1)); continue
...
def report_scan(api,gateway_id,networks):
 observed_at=datetime.now(timezone.utc).isoformat(); payload={"gatewayId":gateway_id,"observedAt":observed_at,"hotspots":[{"gatewayId":gateway_id,"ssid":n["ssid"],"bssid":n["bssid"],"signalDbm":n.get("signalDbm"),"frequency":n.get("frequency"),"security":n.get("security","OPEN"),"observedAt":observed_at} for n in networks]}
...
def main():
 parser=argparse.ArgumentParser(description="NetworkStream Windows Wi-Fi observation gateway"); parser.add_argument("--api",default="http://127.0.0.1:8080"); parser.add_argument("--gateway-id",default=f"WIN-{socket.gethostname()}"); parser.add_argument("--hotspot-id",default=None); parser.add_argument("--scan",action="store_true"); parser.add_argument("--once",action="store_true"); parser.add_argument("--interval",type=int,default=30); args=parser.parse_args()
 print("NetworkStream Windows Gateway Agent",VERSION); print("gateway:",args.gateway_id); print("host:",socket.gethostname()); print("NOTE: observation/control-plane only; no Windows routing or firewall changes")
 ...
 while True:
  try:
   ...
   if args.scan:
    networks=scan_wifi(); print(f"nearby Wi-Fi networks: {len(networks)}")
