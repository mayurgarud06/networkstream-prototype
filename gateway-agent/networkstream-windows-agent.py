...
def post_json(url,payload):
    data=json.dumps(payload).encode("utf-8"); request=urllib.request.Request(url,data=data,headers={"Content-Type":"application/json","Accept":"application/json"},method="POST")
    with urllib.request.urlopen(request,timeout=8) as response:
        body=response.read().decode("utf-8"); return json.loads(body) if body else None
...
def internet_reachable():
    try:
        with urllib.request.urlopen(INTERNET_TEST_URL,timeout=5) as response: return 200 <= response.status < 400
...
def scan_wifi():
    ...
    for raw in result.stdout.splitlines():
        ...
        if m: current_ssid=m.group(1).strip() or "<hidden>"; current=None; current_auth="OPEN"; current_encryption=None; continue
        m=re.match(r"Authentication\s*:\s*(.*)$",line,re.I)
        if m: current_auth=m.group(1).strip() or "OPEN"; continue
        m=re.match(r"Encryption\s*:\s*(.*)$",line,re.I)
        if m: current_encryption=m.group(1).strip(); continue
        m=re.match(r"BSSID\s+\d+\s*:\s*([0-9a-fA-F:-]+)",line,re.I)
        if m and current_ssid is not None:
            security=current_auth if not current_encryption or current_encryption.lower()=="none" else f"{current_auth}/{current_encryption}"
            current={"ssid":current_ssid,"bssid":m.group(1).lower(),"signalDbm":None,"signalPercent":None,"frequency":None,"security":security}; networks.append(current); continue
        if current is None: continue
        m=re.match(r"Signal\s*:\s*(\d+)%",line,re.I)
        if m: current["signalPercent"]=int(m.group(1)); continue
        m=re.match(r"Channel\s*:\s*(\d+)",line,re.I)
        if m: current["frequency"]=channel_to_frequency(m.group(1)); current["channel"]=int(m.group(1))
    return networks
...
def discover_clients():
    ...
    for line in result.stdout.splitlines():
        ...
        if m and m.group(1)!="192.168.137.1": clients.append({"ipAddress":m.group(1),"macAddress":m.group(2).lower().replace("-",":"),"hostname":None})
...
def apply_firewall(ip,allow):
    ...
    name=f"{FIREWALL_PREFIX}{ip}"; inbound=f"{name}-Inbound"
    remove=f"Remove-NetFirewallRule -DisplayName '{name}' -ErrorAction SilentlyContinue; Remove-NetFirewallRule -DisplayName '{inbound}' -ErrorAction SilentlyContinue"
    run(["powershell","-NoProfile","-Command",remove])
    if not allow:
        command=f"New-NetFirewallRule -DisplayName '{name}' -Direction Outbound -RemoteAddress {ip} -Action Block -Profile Any; New-NetFirewallRule -DisplayName '{inbound}' -Direction Inbound -RemoteAddress {ip} -Action Block -Profile Any"
...
def report_scan(api,gateway_id,networks):
    observed_at=datetime.now(timezone.utc).isoformat(); payload={"gatewayId":gateway_id,"observedAt":observed_at,"hotspots":[{"gatewayId":gateway_id,"ssid":n["ssid"],"bssid":n["bssid"],"signalDbm":n.get("signalDbm"),"signalPercent":n.get("signalPercent"),"frequency":n.get("frequency"),"security":n.get("security","OPEN"),"observedAt":observed_at} for n in networks]}; return post_json(f"{api}/api/gateways/{gateway_id}/scan",payload)
def report_telemetry(api,gateway_id,clients,internet_online):
    payload={"gatewayId":gateway_id,"observedAt":datetime.now(timezone.utc).isoformat(),"internetOnline":internet_online,"upstreamInterface":"Wi-Fi","upstreamAddress":None,"downstreamInterface":"Windows Mobile Hotspot","downstreamAddress":"192.168.137.1","clients":clients}; return post_json(f"{api}/api/gateways/{gateway_id}/telemetry",payload)
...
def main():
    parser=argparse.ArgumentParser(description="NetworkStream Windows gateway agent"); parser.add_argument("--api",default="http://127.0.0.1:8080"); parser.add_argument("--gateway-id",default=f"WIN-{socket.gethostname()}"); parser.add_argument("--hotspot-id",default=None); parser.add_argument("--scan",action="store_true"); parser.add_argument("--no-scan",action="store_true"); parser.add_argument("--data-plane",action="store_true"); parser.add_argument("--once",action="store_true"); parser.add_argument("--interval",type=int,default=30); args=parser.parse_args(); scan_enabled=not args.no_scan
    ...
    while True:
        try:
            ...
            if scan_enabled:
                print(f"nearby Wi-Fi networks: {len(networks)}"); [print("  ",network) for network in networks]; print("scan report:",report_scan(args.api,args.gateway_id,networks))
            clients=discover_clients(); print("downstream clients:",clients); print("telemetry:",report_telemetry(args.api,args.gateway_id,clients,online))
